...
function PostDetail() {
  ...
  const handleLike = async () => {
    try {
      const userId = user?.email; // Get userId from session user
      if (!userId) {
        setError("You must be logged in to like a post");
        return;
      }

      const updatedPost = await likePost(id, userId); // Pass userId to likePost
      setPost(updatedPost);
    } catch (err) {
      console.error(err);
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-100">
          ...
          >
            ...
            <span>Like</span>
            {post.likes && (
              <span className="font-medium ml-1">({post.likes.length})</span>
            )}
          </button>
        </div>
