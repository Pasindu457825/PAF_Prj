...
export const createCourse = async (courseData) => {
  try {
    ...
    const formData = courseData instanceof FormData ? 
      courseData : 
      new FormData();
...
export const deleteCourse = async (courseId, userId) => {
  try {
    const response = await axios.delete(`${API_URL}/${courseId}?userId=${userId}`);
    return response.data;
  } catch (error) {
...
export const getCoursesByAuthor = async (authorId) => {
  try {
    ...
    const response = await axios.get(`${API_URL}/author/${authorId}?_=${timestamp}`);
...
export const getCourseUnit = async (courseId, unitIndex) => {
  try {
    const response = await axios.get(`${API_URL}/${courseId}/units/${unitIndex}`);
    return response.data;
  } catch (error) {
    throw error;
  }
