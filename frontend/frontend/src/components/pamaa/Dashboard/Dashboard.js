...
import { getUserEnrollments, getUserEnrollmentStats } from '../../../services/enrollmentService';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (location.state?.newCourseCreated) {
      setSuccessMessage(`Course "${location.state.courseTitle}" was successfully created!`);
  ...
  useEffect(() => {
    if (location.state?.courseUpdated) {
      setSuccessMessage(`Course "${location.state.courseTitle}" was successfully updated!`);
      setActiveTab('created'); // Automatically switch to Created Courses tab
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [coursesData, enrollmentsData, statsData, certificatesData] = await Promise.all([
  ...
  return (
    ...
    >
      ...
      >
        <button 
          className={`tab-button ${activeTab === 'enrolled' ? 'active bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-100'} flex-1 py-2 px-4 rounded-md transition-colors`} 
          onClick={() => setActiveTab('enrolled')}
        >
          Enrolled Courses ({enrolledCourses.length})
        </button>
        <button 
          className={`tab-button ${activeTab === 'created' ? 'active bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-100'} flex-1 py-2 px-4 rounded-md transition-colors`} 
          onClick={() => setActiveTab('created')}
        >
          Created Courses ({createdCourses.length})
        </button>
        <button 
          className={`tab-button ${activeTab === 'certificates' ? 'active bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-100'} flex-1 py-2 px-4 rounded-md transition-colors`} 
      ...
      >
        ...
        {activeTab === 'certificates' && (
          <div className="certificates-section">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Your Certificates</h2>
            {certificates.length > 0 ? (
              <div className="certificates-list grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {certificates.map(certificate => (
                  ...
                  >
                    <h3 className="text-lg font-bold text-gray-800 mb-2">{certificate.course.title}</h3>
              ...
              <div className="empty-state bg-gray-50 rounded-lg p-8 text-center">
                ...
                <p className="text-gray-600 mb-4">You haven't earned any certificates yet. Complete courses to earn certificates!</p>
                <a 
                  href="/courses" 
